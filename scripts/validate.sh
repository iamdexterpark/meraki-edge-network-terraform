...
if [ -d manifests ]; then
  ...
  # Prefer OpenTofu, fall back to Terraform — the deliverable targets the common HCL surface.
  if command -v tofu >/dev/null 2>&1; then TF=tofu
  elif command -v terraform >/dev/null 2>&1; then TF=terraform
  else echo "  ✗ no HCL engine (tofu/terraform)" >&2; fail=1; TF=""; fi
  if [ -n "$TF" ]; then
    (
      cd terraform
      ...
      "$TF" validate
    ) && echo "  ✓ $TF fmt -check + validate" || { echo "  ✗ $TF fmt/validate failed" >&2; fail=1; }
  fi
else
