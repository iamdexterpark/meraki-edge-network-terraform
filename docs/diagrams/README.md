...
|---|---|
| `hero.mermaid` | README hero: the whole story — JSON intent → reconcile → Meraki cloud → edge. |
| `hld_overview.mermaid` | HLD lead: agnostic primitives (config-as-code, reconciler, control plane, trust zones). |
| `architecture_at_a_glance.mermaid` | The module dependency chain (org→network→devices→features), apply order. |
| `lld_topology.mermaid` | LLD lead: concrete topology — named products (MX/MS/MR), VLANs, addresses, provider. |
| `lifecycle.mermaid` | The Terraform lifecycle state machine: backend → plan → apply → drift → destroy. |
